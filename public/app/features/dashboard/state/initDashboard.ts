...
async function fetchDashboard(
  ...
  try {
    switch (args.routeName) {
      ...
      default:
        throw { message: '未知路径' + args.routeName };
    }
  } catch (err) {
    ...
    dispatch(dashboardInitFailed({ message: '仪表盘抓取失败', error: err }));
    console.error(err);
    return null;
  }
}
...
export function initDashboard(args: InitDashboardArgs): ThunkResult<void> {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(dashboardInitFailed({ message: '仪表盘模型创建失败', error: err }));
      console.error(err);
      return;
    }
    ...
    try {
      ...
      dispatch(notifyApp(createErrorNotification('初始化仪表盘失败', err)));
      console.error(err);
    }
