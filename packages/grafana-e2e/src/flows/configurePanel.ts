  ...
  getScenarioContext().then(({ lastAddedDashboardUid }: any) => {
    ...
    if (isExplore) {
      ...
      if (isEdit) {
        ...
        e2e.components.Panels.Panel.headerItems('编辑').click();
      } else {
        e2e.components.PageToolbar.item('添加面板').click();
    ...
    if (!isExplore) {
      ...
      if (changeTitle || visualizationName) {
        if (changeTitle && panelTitle) {
          e2e.components.PanelEditor.OptionsPane.fieldLabel('面板选项标题').type(`{selectall}${panelTitle}`);
        }
    ...
    if (!isExplore) {
      e2e().get('button[title="应用变更并返回仪表盘"]').click();
